...
function updateLeaderboardUI(squads) {
    ...
    if (squads[0]) {
        ...
        document.getElementById('rank-1-score').innerHTML = `${squads[0].score.toLocaleString()}<div class="text-[9px] md:text-xs opacity-80 mt-1 font-sans font-medium tracking-wide normal-case">G: ${squads[0].games_score} | B: ${squads[0].bonus_score}</div>`;
    }
    if (squads[1]) {
        document.getElementById('rank-2-name').innerText = squads[1].name;
        document.getElementById('rank-2-score').innerHTML = `${squads[1].score.toLocaleString()}<div class="text-[8px] md:text-[10px] opacity-80 mt-1 font-sans font-medium tracking-wide normal-case">G: ${squads[1].games_score} | B: ${squads[1].bonus_score}</div>`;
    }
    if (squads[2]) {
        document.getElementById('rank-3-name').innerText = squads[2].name;
        document.getElementById('rank-3-score').innerHTML = `${squads[2].score.toLocaleString()}<div class="text-[8px] md:text-[10px] opacity-80 mt-1 font-sans font-medium tracking-wide normal-case">G: ${squads[2].games_score} | B: ${squads[2].bonus_score}</div>`;
    }
    ...
    list.innerHTML = remaining.map((s, i) => `
        <div class="bg-white p-5 rounded-2xl flex justify-between items-center shadow-sm border border-[#ffdcc6] hover:shadow-md transition-shadow relative overflow-hidden">
            ...
            <div class="flex flex-col items-end">
                ...
                <div class="text-[9px] font-bold opacity-50 mt-1.5 tracking-wider uppercase font-sans">G: ${s.games_score} | B: ${s.bonus_score}</div>
            </div>
        </div>
    `).join('');
}
