...
async function fetchGamesFromDB() {
    ...
    const { data: games, error } = await supabaseClient
        .from('games')
        .select(`id, name, location, status, game_results ( squad_name, rank, points )`)
    ...
    if (!games || games.length === 0) {
        gamesContainer.innerHTML = '<div class="text-center opacity-50 py-10 col-span-full">No games scheduled yet.</div>';
        return;
    }
